...
async def test_initial_state(heating_actuator):
    ...
    assert heating_actuator.state == 0


@pytest.mark.asyncio
async def test_set_state(heating_actuator):
    """Test to set a specific open level of the HeatingActuator."""
    await heating_actuator.set_state(50)
    ...
    assert heating_actuator.state == 50

    # Also checking lower and upper boundaries
    await heating_actuator.set_state(-1)
    heating_actuator._api.set_datapoint.assert_called_with(
        device_id="ABB289613651",
        channel_id="ch0002",
        datapoint="idp0000",
    ...
    assert heating_actuator.state == 0
    await heating_actuator.set_state(120)
    heating_actuator._api.set_datapoint.assert_called_with(
        device_id="ABB289613651",
        channel_id="ch0002",
        datapoint="idp0000",
    ...
    assert heating_actuator.state == 100
...
async def test_refresh_state_from_output(heating_actuator):
    ...
    assert heating_actuator.state == 35
